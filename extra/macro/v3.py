...
class Macro:
    ...
    def export_csv(self):
        ...
        df = pd.DataFrame({
            ...
            "v0":v0,
            "v1":v1,
            "int":[self.wait for x in range(0, len(name))] #just the same value to indicate the wait interval
        })

        filename = self.title.replace(" ", "_").replace("/", "_").replace(".", "_") + ".csv"

        #ask for the directory to save the file in
